        ...
        <div className="mx-auto max-w-md space-y-4">
          ...
          <div>
            ...
            <div className="mt-3 space-y-3">
              <div>
                <label className="block text-sm font-medium text-[#c1c7d3]">닉네임</label>
                <input
                  value={nickname}
                  onChange={(e) => setNickname(e.target.value)}
                  placeholder="2~20자, 한글/영문/숫자/공백/._-"
                  className="mt-1 w-full rounded-lg border border-[#232936] bg-[#10141b] px-3.5 py-2.5 text-sm text-[#e7e9ee] outline-none placeholder:text-[#9aa3b2] focus:ring-2 focus:ring-[#23D3FF]/40"
                />
                {checkingNick && <p className="mt-1 text-xs text-[#9aa3b2]">확인 중…</p>}
                {nickOK === false && <p className="mt-1 text-xs text-red-600">이미 사용중</p>}
                {nickOK === true && <p className="mt-1 text-xs text-green-600">사용 가능</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-[#c1c7d3]">이름</label>
                <input
                  value={legalName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-[#c1c7d3]">생년월일</label>
                  <input
                    type="date"
                    value={birthDate}
                    onChange={(e) => setBirthDate(e.target.value)}
                    max={birthMax}
                    className="mt-1 w-full rounded-lg border border-[#232936] bg-[#10141b] px-3.5 py-2.5 text-sm text-[#e7e9ee] outline-none focus:ring-2 focus:ring-[#23D3FF]/40"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#c1c7d3]">성별</label>
                  <select
                    value={gender}
          ...
          <div>
            ...
            <div className="mt-3 space-y-3">
              <div>
                <label className="block text-sm font-medium text-[#c1c7d3]">이메일</label>
                <input
                  value={email}
                  ...
                  className="mt-1 w-full rounded-lg border border-[#232936] bg-[#10141b] px-3.5 py-2.5 text-sm text-[#e7e9ee] outline-none placeholder:text-[#9aa3b2] focus:ring-2 focus:ring-[#23D3FF]/40"
                />
                {checkingEmail && <p className="mt-1 text-xs text-[#9aa3b2]">확인 중…</p>}
                {emailOK === false && <p className="mt-1 text-xs text-red-600">이미 사용중</p>}
                {emailOK === true && <p className="mt-1 text-xs text-green-600">사용 가능</p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-[#c1c7d3]">비밀번호</label>
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-3 text-xs text-[#9aa3b2]"
                  >
                    {showPw ? "숨김" : "보기"}
                  </button>
                </div>
          ...
          <div>
            ...
            <div className="mt-3 space-y-2 text-sm text-[#c1c7d3]">
          ...
          <button
            ...
            onClick={submit}
            disabled={submitting}
            className="w-full rounded-lg bg-[#23D3FF] px-4 py-3 text-sm font-semibold text-[#151a22] disabled:opacity-60"
          >
